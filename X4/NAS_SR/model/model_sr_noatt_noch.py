...
OPS = {
    ...
    'wdsr1': lambda exp, C, stride, affine: WDSR1(C, exp, C, 3, None, act=nn.ReLU(True)),
    'wdsr2': lambda exp, C, stride, affine: WDSR2(C, C, exp, 3, None, act=nn.ReLU(True)),
    'wdsr3': lambda exp, C, stride, affine: WDSR3(C, exp, C, 3, None, act=nn.ReLU(True)),
    'wdsr4': lambda exp, C, stride, affine: WDSR4(C, C, exp, 3, None, act=nn.ReLU(True)),
...
class WDSR3(nn.Module):
    def __init__(self, n_feats, exp, c2, kernel_size, wn, act):
        super(WDSR3, self).__init__()
        body = []
        body.append(nn.Conv2d(n_feats, exp[0], 1, padding=1 // 2))
        body.append(act)
        body.append(nn.Conv2d(exp[0], exp[1], 1, padding=1 // 2))
...
class WDSR4(nn.Module):
    def __init__(self, n_feats, c1, exp, kernel_size, wn, act):
        super(WDSR4, self).__init__()
        body = []
        body.append(nn.Conv2d(n_feats, exp[0], kernel_size, padding=kernel_size // 2))
        body.append(act)
        body.append(nn.Conv2d(exp[0], exp[1], 1, padding=1 // 2))
    ...
    def __init__(self, args, conv=common.default_conv):
        ...
        for i in range(self.total_cell_num):
            ...
            if op_names[0] == 'rcab':
                ...
                ch1, ch2 = geno_channel[i]
                exp.append([self.G1, self.G2])
            elif op_names[0] == 'wdsr3':
                ch1, ch2 = geno_channel[i]
                exp.append([self.G1, self.G2])
            elif op_names[0] == 'wdsr2':
                ch1, ch2 = geno_channel[i]
                exp.append([self.G2, self.G1])
            elif op_names[0] == 'wdsr4':
                ch1, ch2 = geno_channel[i]
                exp.append([self.G2, self.G1])
            elif op_names[0] == 'resb':
                ch1, ch2 = geno_channel[i]
                exp.append([self.G2, self.G2])
