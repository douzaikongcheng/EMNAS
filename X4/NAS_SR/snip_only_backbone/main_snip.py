...
def apply_prune_mask(net, keep_masks):
    ...
    prunable_layers = filter(
        lambda layer: (isinstance(layer, nn.Conv2d) and layer.weight.data.shape != torch.Size([3, 3, 1, 1]) and layer.weight.data.shape != torch.Size([32, 3, 3, 3]) and layer.weight.data.shape != torch.Size([48, 12, 3, 3]) and layer.weight.data.shape != torch.Size([3, 32, 3, 3])) or isinstance(layer, nn.Linear), net.modules())
...
def main():
    ...
    if args.data_test == 'video':
        ...
        if checkpoint.ok:
            ...
            print(share_model.model.cells[0].op.body[0].weight)
            # print(share_model.model.body[0].weight)
