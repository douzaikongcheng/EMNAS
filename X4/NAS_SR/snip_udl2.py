...
def SNIP(net, keep_ratio, train_dataloader, device):
    ...
    for layer in net.modules():
        if (isinstance(layer, nn.Conv2d) and layer.weight.data.shape != torch.Size([3, 3, 1, 1])) or isinstance(layer, nn.Linear):
            layer.weight_mask = nn.Parameter(torch.ones_like(layer.weight))
            # nn.init.xavier_normal_(layer.weight)
            layer.weight.requires_grad = False

        # Override the forward methods:
        if (isinstance(layer, nn.Conv2d) and layer.weight.data.shape != torch.Size([3, 3, 1, 1])):
    ...
    # print('-------------------------------')
    # print(net)
    # print(np.sum(np.prod(v.size()) for name, v in net.named_parameters()))
