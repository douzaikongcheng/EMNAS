...
class Data:
    def __init__(self, args):
        ...
        if not args.test_only:
            ...
            # # self.loader_train = MSDataLoader(
            # #     args,
            # self.loader_val_train = DataLoader(
            #     MyConcatDataset(datasets),
            #     batch_size=args.batch_size,
            #     num_workers=args.n_threads,
            #     shuffle=True,
        ...
        for d in args.data_test:
            if d in ['Set5', 'Set14','Set12','BSD200', 'BSD68','BSD100', 'Urban100','Manga109','DIV2K']:
        ...
        # for d in ['DIV2K']:
        for d in args.data_train:
            if d in ['Set5', 'Set14', 'Set12','BSD200','BSD100', 'Urban100']:
                m = import_module('data.benchmark')
                testset = getattr(m, 'Benchmark')(args, train=False, name=d)
            else:
                module_name = args.data_train[0]
                m = import_module('data.' + module_name.lower())
                testset.append(getattr(m, module_name)(args,train=False, name=d))
                # testset = getattr(m, module_name)(args, train=False, name=d)
            # print("test: module_name.lower():", module_name.lower())
            # self.loader_val_train = MSDataLoader(
            #     args,
            self.loader_val_train = DataLoader(
