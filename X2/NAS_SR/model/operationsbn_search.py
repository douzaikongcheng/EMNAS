...
# Conv2d_with_parameters group = 8
class Conv2dWp8(nn.Module):
    def __init__(self, in_channels, out_channels,
                 kernel_size, padding, stride=1, groups=1, bias=True):
        super(Conv2dWp8, self).__init__()
        self.conv2d = nn.Conv2d(in_channels, out_channels, kernel_size, padding=padding, stride=stride,
                                groups=groups, bias=bias)
        self.theta = torch.nn.Parameter(torch.ones(out_channels // 8), requires_grad=True)

    def forward(self, x):
        out = self.conv2d(x)
        c = out.shape[1]
        # p = self.theta.unsqueeze(0).unsqueeze(2).unsqueeze(3)
        for i in range(0, c, 8):
            out[:, i:i + 8, :, :, ] *= self.theta[i // 8]
        return out


# Conv2d_with_parameters group = 4
class Conv2dWp4(nn.Module):
    def __init__(self, in_channels, out_channels,
                 kernel_size, padding, stride=1, groups=1, bias=True):
        super(Conv2dWp4, self).__init__()
        self.conv2d = nn.Conv2d(in_channels, out_channels, kernel_size, padding=padding, stride=stride,
                                groups=groups, bias=bias)
        self.theta = torch.nn.Parameter(torch.ones(out_channels // 4), requires_grad=True)
...
class W1(nn.Module):
    def __init__(self, n_feats, c1, c2, kernel_size, wn, act):
        ...
        body = []
        body.append(Conv2dWp8(n_feats, c1, 1, padding=1 // 2))
        body.append(act)
        body.append(Conv2dWp4(c1, c2, 1, padding=1 // 2))
...
class W2(nn.Module):
    def __init__(self, n_feats, c1, c2, kernel_size, wn, act):
        ...
        body = []
        body.append(Conv2dWp4(n_feats, c1, kernel_size, padding=kernel_size // 2))
        body.append(Conv2dWp8(c1, c2, 1, padding=1 // 2))
        body.append(act)
...
class RCAB(nn.Module):
    def __init__(
        ...
        modules_body = []
        modules_body.append(Conv2dWp4(n_feat, c1, kernel_size, padding=1))
...
class Resb(nn.Module):
    def __init__(self, n_feats, c1, c2, kernel_size, wn, act):
        ...
        body = []
        body.append(Conv2dWp4(n_feats, c1, 1, padding=1 // 2))
        body.append(act)
        body.append(Conv2dWp4(c1, c2, kernel_size, padding=kernel_size // 2))
        body.append(act)
        body.append(nn.Conv2d(c2, n_feats, 1, padding=1 // 2))
        self.body = nn.Sequential(*body)
