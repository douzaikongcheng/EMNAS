    ...
    def train(self):
        ...
        for batch, (lr, hr, _) in enumerate(self.loader_train):
            ...
            for p in self.model.parameters():
                if p.requires_grad and len(p.shape) >= 4 and (p.shape != torch.Size([self.args.n_feats, self.args.n_colors, 3, 3])) and (p.shape != torch.Size([self.args.tail_fea * self.args.scale[0] * self.args.scale[0], self.args.tail_fea, 3, 3])) and (p.shape != torch.Size([self.args.n_colors, self.args.tail_fea, 3, 3])):
                    l1_penalty += p.abs().sum()
                    # print(p.shape)
            # loss = self.loss(sr, hr) + 0.01 * l1_penalty   # (Best: 35.067 @epoch 22) 100多epoch还是35db
            # loss = self.loss(sr, hr) + 0.001 * l1_penalty   # (Best: 35.184 @epoch 184) 100多epoch还是35db

            loss = self.loss(sr, hr) + self.args.sparse * l1_penalty  # finetune的时候不应加这个
